...
- (void)gameChanged:(NSNotification *)notification {
    ...
    if (_state == menuState_gameRunning && game.gameState==kFFGameState_Won){
        ...
        if ([game isRandomChallenge]){
            ...
            [FFAnalytics log:@"CHALLENGE_WON" with:[NSDictionary dictionaryWithObjectsAndKeys:
                    @(level),@"LEVEL",
...
- (void)showChallengeUnlockToastIfNecessary {
    ...
    for (NSUInteger i = 0; i < [[FFGamesCore instance] autoGeneratedLevelCount]; i++){
...
- (void)startHotSeatGame {
    ...
    [FFAnalytics log:@"HOT_SEAT_GAME_STARTED"];
...
- (void)giveUpAndBackToChallengeMenu {
    ...
    if ([selectedGame isRandomChallenge]){
        ...
        [FFAnalytics log:@"CHALLENGE_ABORTED"
                    with:[NSDictionary dictionaryWithObjectsAndKeys:@(_currentRandomChallenge+1),@"CHALLENGE",nil]];
    } else {
        [self changeState:menuState_choosePuzzle];

        [FFAnalytics log:@"PUZZLE_ABORTED"
...
- (void)activatePuzzleAtIndex:(NSUInteger)i {
    if (![FFStorageUtil isUnlocked] && i >= FIRST_PUZZLE_TO_UNLOCK){
...
- (void)activateRandomChallengeAtIndex:(NSUInteger)i {
    if (![FFStorageUtil isUnlocked] && i >= FIRST_CHALLENGE_TO_UNLOCK){
        [self.delegate gotoStore];
    } else {
        ...
        [FFAnalytics log:@"CHALLENGE_START"
                    with:[NSDictionary dictionaryWithObjectsAndKeys:@(_currentRandomChallenge+1),@"CHALLENGE", nil]];
    }
}
...
- (void)rematch {
    [FFAnalytics log:@"HOT_SEAT_REMATCH_TAPPED"];
    [self restartGame];
}
